...
//		'нё ' => 0,
	...
	private function _word(array &$a)
	{
		...
		{
			if (($this->mode & self::SIMILAR_CHARS) === 0) {
                return $word;
            }
		...
		elseif (! empty($a[1]) && strlen($word) >= 4)
		{
			...
			if (preg_match('/^(?:' . $this->en_uc . '|' . $this->sc . '){1,6}+$/sxSX', $word)) {
                return $word;
            }
		...
		if ($c === 0) {
            ...
            $s = $this->_detect($word, $suggestions, ! empty($a[3]));
        }
		if ($s !== $word)
		{
			$this->words[$word] = $s;
	...
	private function _bigram_exists($word, $lang)
	{
		...
		if (preg_match('/(?:' . $this->consonant_lc[$lang] . '){4}/sxSX', $word, $m)
			...
			&& ! array_key_exists($m[0], $this->consonants4_lc[$lang]))
        {
            return true;
        }
